...
//		ctx.executor().execute(() -> ctx.pipeline().fireUserEventTriggered(reader));
	...
	private void enqueueAvailableReader(final NetworkSequenceViewReader reader) throws Exception {
		...
		if (triggerWrite) {
			writeAndFlushNextMessageIfPossible(endpoint);
		}
	}
...
//		ctx.pipeline().fireUserEventTriggered(receiverId);
	...
	private void writeAndFlushNextMessageIfPossible(final RdmaShuffleServerEndpoint endpoint) throws IOException {
		...
		try {
			while (true) {
				...
				// TODO (venkat): we should not return here
	...
	private void handleException(RdmaShuffleServerEndpoint endpoint, Throwable cause) throws IOException {
