...
public class RdmaServerRequestHandler implements Runnable {
	...
	private final Map<Long, NettyMessage.BufferResponse> inFlightDataSends =new HashMap<>();
	...
	public void run() {
		while (!stopped) {
			try {
				LOG.info("Server accepting connection and setting up endpoint");
				RdmaShuffleServerEndpoint clientEndpoint = serverEndpoint.accept();
				LOG.info("Server accepted connection and setting up endpoint");
	...
	private NettyMessage.BufferResponse readPartition(NettyMessage.PartitionRequest request, NetworkSequenceViewReader
		reader) throws
		IOException,
		...
		while (true) {
			...
			if (next == null) {
				...
				LOG.error("Encountered error but silently ignoring ", cause);
//				if (cause != null) {
...
//								}
								...
								NettyMessage.PartitionRequest partitionRequest = (NettyMessage.PartitionRequest)
									clientRequest;
								LOG.info("received partition request: " + partitionRequest.receiverId + "at endpoint: " +
								...
								while (!reader.isRegisteredAsAvailable()) {
									LOG.info("waiting for partition" + partitionRequest.partitionId + " at the " +
										"endpoint for data availability: " + getEndpointStr(clientEndpoint) + "reader " +
										"status:" + reader);
									synchronized (reader) {
										reader.wait();
									}
								...
								NettyMessage.BufferResponse response = readPartition(partitionRequest, reader);
...
//								clientEndpoint.getSendBuffer().put(response.write(bufferPool).nioBuffer());
								...
								// receive
								RdmaSendReceiveUtil.postSendReqServer(clientEndpoint, ++workRequestId, response.getBufferAddress() );
								inFlightDataSends.put(workRequestId,response);
...
//							RdmaSendReceiveUtil.postSendReq(clientEndpoint, ++workRequestId);
							...
							inFlightDataSends.get(wc.getWr_id()).releaseBuffer();
							inFlightDataSends.remove(wc.getWr_id());
						}
